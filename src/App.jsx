...
function EditableField({ value, onSave, multiline = false, textClass = "", accentColor, cycleAccent, isUnlocked, requireAuth }) {
  ...
  if (editing) {
    return multiline ? (
      <textarea 
        autoFocus value={val} onChange={e => setVal(e.target.value)} onBlur={handleSave} onKeyDown={handleKeyDown} 
      ...
      <input 
        autoFocus type="text" value={val} onChange={e => setVal(e.target.value)} onBlur={handleSave} onKeyDown={handleKeyDown} 
  ...
  return (
    <div 
      className="group relative cursor-pointer flex-1 flex items-center min-h-[24px] w-full" 
      onTouchStart={startPress} onTouchEnd={endPress} onMouseDown={startPress} onMouseUp={endPress} onMouseLeave={endPress} 
      title={isUnlocked ? "Clique no ícone ou segure para editar" : "Desbloqueie para editar"}
    >
       <span className={textClass}>{value || '-'}</span>
       <button 
         onClick={handleEditClick} 
         className={`absolute -right-5 top-0 opacity-0 group-hover:opacity-100 p-1 rounded-sm shadow-md transition-opacity ${isUnlocked ? 'bg-black text-white' : 'theme-crimson'}`}
       >
...
function EditableSelect({ value, options, onSave, textClass = "", isStatus = false, accentColor, cycleAccent, isUnlocked, requireAuth }) {
  ...
  if (editing) {
    return (
      <select 
        ...
        className={`p-1 border-[2px] outline-none transition-colors duration-300 bg-white text-black`} 
        style={{ borderColor: accentColor }}
      >
  ...
  return (
    <div 
      className="group relative cursor-pointer flex items-center w-max" 
    ...
    >
       ...
       <button 
         onClick={handleEditClick} 
         className={`opacity-0 group-hover:opacity-100 p-1 ml-1 rounded-sm shadow-md transition-opacity ${isUnlocked ? 'bg-black text-white' : 'theme-crimson'}`}
       >
         {isUnlocked ? <Edit2 size={12} /> : <Lock size={12} />}
       </button>
    </div>
    ...
    <div className={`min-h-screen font-sans ${themeConfig.bg} ${themeConfig.text} text-xs transition-colors duration-300 flex flex-col`}>
      ...
      <header className={`flex flex-col md:flex-row border-b-[6px] ${themeConfig.border} bg-white`}>
        <div className={`flex-1 p-4 md:p-6 ${bMedium} border-b-0 md:border-b-0 md:border-r-[6px] flex items-center justify-between`}>
          ...
          >
            <img 
              src="https://raw.githubusercontent.com/killuixo/tabulum-sig/refs/heads/main/icon-192.png" 
              alt="TABULUM Icon" 
            ...
            <div>
              ...
              <p className="font-bold opacity-60 uppercase tracking-widest text-[0.7em] mt-1">Gestão de Utilidade Pública</p>
            </div>
          </div>
          <div className="hidden md:flex gap-2 items-center">
            ...
            <div className="w-8 h-8 ml-2" style={{ backgroundColor: COLORS.cyan, border: `3px solid #000` }}></div>
            <div className="w-8 h-8" style={{ backgroundColor: COLORS.mustard, border: `3px solid #000` }}></div>
            <div className="w-8 h-8" style={{ backgroundColor: COLORS.crimson, border: `3px solid #000` }}></div>
          </div>
        </div>
        ...
        <nav className={`flex flex-wrap md:flex-nowrap p-3 md:p-4 gap-3 overflow-x-auto bg-white items-center md:justify-center`}>
          ...
          <button 
            ...
            className={`flex items-center justify-center font-black text-2xl w-12 h-12 border-[4px] transition-all duration-300 hover:-translate-y-1 flex-shrink-0 bg-white text-black hover:bg-black hover:text-white`}
            style={isFormOpen 
              ? { borderColor: accentColor, boxShadow: `4px 4px 0px ${accentColor}`, backgroundColor: 'black', color: 'white', transform: 'scale(1.05)', zIndex: 10 } 
          ...
          <>
            {isFormOpen && (
              <FormNovoPedido 
                ...
                fetchFromWebhooks={() => fetchFromWebhooks()} equipeOptions={articuladoresUnicos} 
                webhookUtilidade={webhookUtilidade} emailCentral={EMAIL_CENTRAL} 
                accentColor={accentColor} cycleAccent={cycleAccent} requireAuth={requireAuth} 
                showAlert={showAlert} isUnlocked={isUnlocked}
              />
            )}
            
            {!isFormOpen && view === 'entity_details' && activeFicha && (
              <FichaEntidade 
                item={activeFicha} equipeOptions={articuladoresUnicos} emailCentral={EMAIL_CENTRAL}
                ...
                theme={themeConfig} thick={bThick} accentColor={accentColor} cycleAccent={cycleAccent}
            ...
            {!isFormOpen && view === 'articulator_details' && activeArticulador && (
              <PainelArticulador 
                ...
                onClose={() => {setActiveArticulador(null); setView('kanban'); cycleAccent();}} 
                onEntidadeClick={handleEntityClick} theme={themeConfig} thick={bThick} 
              />
            )}

            {!isFormOpen && view === 'kanban' && (
              <KanbanView data={data} theme={themeConfig} thick={bThick} med={bMedium} onEntityClick={handleEntityClick} onArticulatorClick={handleArticulatorClick} />
            )}

            {!isFormOpen && view === 'dashboard' && (
              <DashboardView data={data} theme={themeConfig} thick={bThick} med={bMedium} onEntityClick={handleEntityClick} onArticulatorClick={handleArticulatorClick} />
            )}
          </>
        )}
      </main>
        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in zoom-in duration-200">
           <div className={`w-full max-w-sm p-6 flex flex-col gap-4 ${bThick} bg-white shadow-[8px_8px_0px_rgba(0,0,0,0.5)] border-[3px]`} style={{ borderColor: accentColor }}>
              ...
              <input 
                 ...
                 className={`w-full p-3 border-[3px] outline-none font-bold transition-colors duration-300 bg-gray-100 text-black`}
                 ...
                 onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                       ...
                       if (val === masterPassword || val.trim().toLowerCase() === EMERGENCY_PHRASE.toLowerCase()) {
                          ...
                          showAlert('Acesso Negado: Senha ou Frase de Emergência Incorreta.');
                       }
                    }
                 }}
                 id="auth_pwd_input"
              ...
              <div className="flex gap-2 mt-2">
                 <button 
                    ...
                    className="flex-1 p-3 bg-black text-white font-black uppercase tracking-widest text-[0.8em] border-[3px] border-black hover:-translate-y-1 transition-transform"
                 >
                   Desbloquear
                 </button>
                 <button 
                    onClick={() => { setAuthModalOpen(false); setPendingAction(null); }}
                    className="flex-1 p-3 bg-transparent text-black font-black uppercase tracking-widest text-[0.8em] border-[3px] border-black hover:-translate-y-1 transition-transform hover-crimson"
                 >
                   Cancelar
                 </button>
              </div>
      ...
      {dialog.isOpen && (
        <div className="fixed inset-0 z-[300] flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in zoom-in duration-200">
          <div 
            className={`w-full max-w-sm p-6 flex flex-col gap-4 border-[4px] bg-white shadow-[8px_8px_0px_rgba(0,0,0,0.5)]`} 
            style={{ borderColor: dialog.type === 'confirm' ? COLORS.mustard : COLORS.crimson }}
          >
            <div className="flex items-center gap-3 border-b-[4px] border-black pb-2">
              ...
              <h2 className={`text-xl font-black uppercase tracking-widest ${dialog.type === 'confirm' ? 'text-mustard' : 'text-crimson'}`}>
                {dialog.type === 'confirm' ? 'Atenção' : 'Aviso'}
              </h2>
            </div>
            <p className="text-sm font-bold opacity-90 leading-relaxed whitespace-pre-wrap text-black">
              {dialog.message}
            </p>
            <div className="flex gap-2 mt-2">
              {dialog.type === 'confirm' && (
                <button 
                  onClick={() => { setDialog({ isOpen: false }); dialog.onConfirm(); }}
                  className="flex-1 p-3 bg-black text-white font-black uppercase tracking-widest text-[0.8em] border-[3px] border-black hover:-translate-y-1 transition-transform"
                >
                  Confirmar
                </button>
              ...
              <button 
                ...
                className="flex-1 p-3 bg-transparent text-black font-black uppercase tracking-widest text-[0.8em] border-[3px] border-black hover:-translate-y-1 transition-transform hover-crimson"
              >
                {dialog.type === 'confirm' ? 'Cancelar' : 'OK'}
              </button>
            </div>
          </div>
        </div>
  ...
  const renderCard = (item, i, colColor) => {
    ...
    return (
      <div 
        ...
        className={`p-3 md:p-4 ${med} hover:-translate-y-1 hover:shadow-[4px_4px_0px_rgba(0,0,0,0.3)] transition-all cursor-pointer bg-[#f4f4f0] text-black`}
      >
        ...
        <div className="flex justify-between items-end gap-2">
          <div className="flex flex-col">
            ...
            <span 
              onClick={(e) => { e.stopPropagation(); onArticulatorClick(item.ARTICULADOR); }} 
              className={`font-bold hover:underline decoration-2 underline-offset-4 cursor-pointer`} 
              style={{ textDecorationColor: colColor || 'currentcolor' }}
            >
              {item.ARTICULADOR || '-'}
            </span>
          </div>
        ...
        {(item['LINK']) && (
          <a 
            ...
            className="mt-3 flex w-max items-center gap-1 text-[0.7em] font-black uppercase tracking-widest opacity-80 hover:opacity-100 hover:underline transition-all" 
            style={{ color: colColor || 'currentcolor' }} 
    ...
    <div className="flex flex-col gap-4 flex-1 min-h-[500px]">
      <div className={`border-[4px] transition-colors duration-300 ${isConcluidoOpen ? 'bg-white' : 'bg-transparent'} ${theme.border}`}>
        <button 
          ...
          className={`w-full p-3 font-black uppercase tracking-widest flex items-center justify-between transition-colors ${isConcluidoOpen ? 'bg-black text-white' : 'opacity-60 hover:opacity-100 hover:bg-black/5'}`}
        >
          ...
          return (
            <div key={col.id} className={`flex flex-col transition-all duration-300 ${isCollapsed ? 'w-16 md:w-20' : 'flex-1'} ${thick} bg-white`}>
              <div 
                ...
                className={`p-3 font-black flex items-center gap-2 uppercase tracking-wider border-b-[4px] cursor-pointer transition-colors ${theme.border}`} 
                style={{ backgroundColor: col.color, color: headerTextColor }}
              >
                {isCollapsed ? (
                  ...
                  <>
                    ...
                    <span className="ml-auto bg-black text-white px-2 py-0.5 rounded-full text-[0.8em]">{colData.length}</span>
                  </>
                )}
              </div>
    ...
    <div className="grid grid-cols-1 lg:grid-cols-4 gap-4">
      ...
      <div className={`lg:col-span-4 mt-4 ${thick} bg-white overflow-x-auto max-h-[500px]`}>
        <table className="w-full text-left border-collapse min-w-[600px]">
          ...
          <tbody>
            {data.map((row, i) => (
              <tr key={i} className={`border-b-[3px] ${theme.border} hover:bg-gray-200 transition-colors`}>
                ...
                <td className="p-4 border-r-[4px] border-current">
                   <span 
                     className="px-2 py-1 font-black uppercase text-[0.75em] tracking-widest" 
                     style={{ 
                       backgroundColor: getStatusColor(row['STATUS DA ANÁLISE']) || 'black', 
                       color: getTextColorForStatus(getStatusColor(row['STATUS DA ANÁLISE'])) || 'black'
                     }}
                   >
...
function FichaEntidade({ item, onClose, onArticuladorClick, onDelete, onUpdate, theme, thick, equipeOptions, emailCentral, accentColor, cycleAccent, isUnlocked, requireAuth }) {
  ...
  const handleTouchStart = (key) => {
    pressTimer = setTimeout(() => setActiveTooltip(key), 400); 
  };
    ...
    <div className={`p-6 md:p-8 ${thick} bg-white flex flex-col gap-6 relative animate-in fade-in zoom-in-95 duration-200`}>
      ...
      <div className="pr-10 border-b-[6px] border-current pb-4">
        ...
        <div className="flex flex-wrap gap-4 mt-3">
          <span className="font-bold uppercase tracking-widest text-[0.8em] px-3 py-1 border-[2px] transition-colors duration-500" style={{ backgroundColor: statusColor || 'transparent', color: getTextColorForStatus(statusColor) || 'black', borderColor: statusColor || 'currentcolor' }}>
          ...
          {(item['LINK']) && (
             <a 
               ...
               target="_blank" 
               rel="noopener noreferrer" 
               className={`flex items-center gap-2 font-black uppercase tracking-widest text-[0.8em] px-3 py-1 border-[2px] border-current transition-colors opacity-70 hover:opacity-100`} 
               title="Acompanhar Tramitação"
             >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col mt-4">
            <a 
              href={`mailto:${emailCentral}?subject=${encodeURIComponent(`Pedido de Utilidade Pública de ${item.ENTIDADE}`)}`} 
              className={`mb-6 p-4 border-[3px] border-current flex items-center justify-center gap-3 font-black uppercase tracking-widest text-[0.8em] transition-transform hover:-translate-y-1 bg-black text-white hover-cyan`}
            >
            ...
            {DOCS_KEYS.map((key, idx) => {
              ...
              return (
                <div 
                  key={idx} 
                  className="group relative flex items-center gap-3 py-2 border-b-[2px] border-current opacity-90 cursor-pointer hover:bg-black/5 transition-colors" 
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mt-4 border-t-[4px] border-current pt-4 gap-4">
        <button 
          onClick={() => requireAuth(onDelete)} 
          className={`flex items-center gap-2 px-4 py-3 font-black uppercase tracking-widest text-[0.8em] border-[3px] border-current opacity-50 hover:opacity-100 transition-all w-full sm:w-auto justify-center hover-crimson`}
        >
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in duration-200">
      <div className={`w-full max-w-4xl p-6 md:p-8 flex flex-col gap-6 ${thick} bg-white shadow-[8px_8px_0px_currentColor] relative max-h-[95vh] overflow-hidden`}>
        ...
        <div className="border-t-[4px] border-current pt-4 mt-2 flex-shrink-0">
          <button onClick={onClose} className="w-full p-4 bg-transparent font-black uppercase tracking-widest hover:-translate-y-1 transition-transform border-[4px] border-current hover:bg-black hover:text-white">
            Fechar Manual
          </button>
        </div>
      </div>
  ...
  return (
    <div className={`p-6 md:p-8 ${thick} bg-white flex flex-col gap-6 relative animate-in fade-in zoom-in-95 duration-200 min-h-[60vh]`}>
      ...
      <div className="pr-10 border-b-[6px] border-current pb-4 flex items-center gap-4">
        <div className="w-16 h-16 border-[4px] border-current bg-black text-white flex items-center justify-center font-black text-3xl uppercase">
          {nome.charAt(0)}
        </div>
        <div>
...
function FormNovoPedido({ onClose, theme, thick, fetchFromWebhooks, equipeOptions, webhookUtilidade, emailCentral, accentColor, cycleAccent, requireAuth, showAlert }) {
  ...
  const handleSubmit = async (e) => {
    ...
    requireAuth(async () => {
      if (!webhookUtilidade) { 
        showAlert("Erro de Sistema: Webhook Central Ausente. Verifique os Ajustes."); 
        return; 
      }
      if (!formData.ENTIDADE.trim()) { 
        showAlert("O nome da entidade é obrigatório."); 
        return; 
      }
      ...
      try {
        const payload = {
          ...
          "2 ATA DE ELEIÇÃO/POSSE": docs['2 ATA DE ELEIÇÃO/POSSE'] ? "TRUE" : "FALSE",
          "3 CNPJ": docs['3 CNPJ'] ? "TRUE" : "FALSE",
          "4 DECLARAÇÃO NÃO OSCIP": docs['4 DECLARAÇÃO NÃO OSCIP'] ? "TRUE" : "FALSE",
          "5 DECLARAÇÃO FUNCIONAMENTO": docs['5 DECLARAÇÃO FUNCIONAMENTO'] ? "TRUE" : "FALSE",
          "6 - 7 DECLARAÇÃO REMUNERAÇÃO": docs['6 - 7 DECLARAÇÃO REMUNERAÇÃO'] ? "TRUE" : "FALSE",
          "8 ESTATUTO": docs['8 ESTATUTO'] ? "TRUE" : "FALSE",
          "9 RELATÓRIO DE ATIVIDADES": docs['9 RELATÓRIO DE ATIVIDADES'] ? "TRUE" : "FALSE",
        ...
        console.error(error); 
        showAlert("Erro ao comunicar com o Arquivo Central."); 
      } finally { 
        setSending(false); 
      }
    });
  ...
  if (successMode) {
    return (
      <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-in zoom-in duration-300">
        <div className={`w-full max-w-lg p-8 flex flex-col gap-6 text-center ${thick} bg-white shadow-[8px_8px_0px_currentColor] border-[3px]`} style={{ borderColor: COLORS.cyan }}>
    ...
    <div className="absolute inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
      <div className={`w-full max-w-3xl p-6 md:p-8 flex flex-col gap-6 ${thick} bg-white shadow-[8px_8px_0px_rgba(0,0,0,0.3)] relative overflow-y-auto max-h-[90vh]`}>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b-[4px] border-current pb-2 pr-8 gap-4">
          ...
          <button 
            type="button" 
            onClick={() => {setIsManualOpen(true); cycleAccent();}} 
            className="flex items-center gap-2 px-3 py-2 bg-black text-white font-black uppercase tracking-widest text-[10px] hover:-translate-y-1 transition-transform shadow-[2px_2px_0px_currentColor]"
          >
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-6">
          <div className={`p-4 border-[3px] border-current bg-[#f4f4f0] flex flex-col gap-4`}>
            ...
            <div className="flex flex-col gap-1 relative">
              ...
              <div className="relative">
                <input 
                  type="text" 
                  value={busca} 
                  onChange={e => { setBusca(e.target.value); setFormData({...formData, ARTICULADOR: e.target.value}); setShowDropdown(true); }} 
                  onFocus={() => {setShowDropdown(true); handleFocus('ARTICULADOR');}} 
                  onBlur={() => setTimeout(()=>setFocusedField(null), 200)} 
                  className={`w-full p-3 border-[3px] outline-none font-bold pr-10 transition-colors duration-300 bg-white text-black`} 
                  style={{ borderColor: focusedField === 'ARTICULADOR' ? accentColor : 'currentcolor' }} 
              ...
              {showDropdown && (
                <div className={`absolute top-full left-0 right-0 mt-1 border-[3px] z-10 max-h-40 overflow-y-auto bg-white`} style={{ borderColor: accentColor }}>
                  {filteredEquipe.map((nome, idx) => (
                    <div 
                      key={idx} 
                      onClick={() => { setFormData({...formData, ARTICULADOR: nome}); setBusca(nome); setShowDropdown(false); }} 
                      className={`p-2 font-bold cursor-pointer hover:bg-black/10 transition-colors border-b border-current opacity-60 hover:opacity-100`}
                    >
                      {nome}
                    </div>
                  ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="flex flex-col gap-1">
                ...
                <input 
                  ...
                  onBlur={() => setFocusedField(null)} 
                  className={`p-3 border-[3px] outline-none font-bold transition-colors duration-300 bg-white text-black`} 
                  style={{ borderColor: focusedField === 'EMAIL' ? accentColor : 'currentcolor' }} 
                />
              </div>
              <div className="flex flex-col gap-1">
            ...
            <div className="flex flex-col gap-1">
              ...
              <input 
                type="url" 
                value={formData['DOCUMENTOS NO DRIVE']} 
                onChange={e => setFormData({...formData, 'DOCUMENTOS NO DRIVE': e.target.value})} 
                onFocus={() => handleFocus('DRIVE')} 
                onBlur={() => setFocusedField(null)} 
                className={`p-3 border-[3px] outline-none font-bold transition-colors duration-300 bg-white text-black`} 
                style={{ borderColor: focusedField === 'DRIVE' ? accentColor : 'currentcolor' }} 
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea 
                ...
                onBlur={() => setFocusedField(null)} 
                className={`p-3 border-[3px] outline-none font-bold resize-none transition-colors duration-300 bg-white text-black`} 
                style={{ borderColor: focusedField === 'OBSERVAÇÕES' ? accentColor : 'currentcolor' }}
              />
            </div>

            <div className="flex flex-col gap-2 mt-2">
              <a 
                href={`mailto:${emailCentral}?subject=${encodeURIComponent(`Pedido de Utilidade Pública de ${formData.ENTIDADE || '[NOME DA ENTIDADE]'}`)}`} 
                className={`p-4 border-[3px] border-current flex items-center justify-center gap-3 font-black uppercase tracking-widest text-[0.8em] transition-transform hover:-translate-y-1 bg-black text-white hover-cyan`}
              >
                <Mail size={18} /> Enviar Documentos por E-mail
              </a>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {DOCS_KEYS.map((key, idx) => {
                  ...
                  return (
                    <div key={idx} className={`p-2 border-[2px] border-current flex items-center gap-3 transition-colors ${docs[key] ? 'bg-black/5' : 'bg-transparent'}`}>
                      <input 
                        type="checkbox" 
                        id={`doc-${idx}`}
                        checked={!!docs[key]} 
          ...
          <button 
            type="submit" 
            disabled={sending} 
            className={`p-5 font-black uppercase tracking-widest text-lg border-[4px] border-current transition-all shadow-[6px_6px_0px_currentColor] flex items-center justify-center gap-3 ${sending ? 'opacity-50 shadow-none translate-y-1 translate-x-1' : 'active:shadow-none active:translate-y-1 active:translate-x-1'}`} 
            style={{ backgroundColor: accentColor, color: getTextColorForStatus(accentColor) }}
          >
            <Lock size={20}/> {sending ? 'SALVANDO...' : 'SALVAR'}
          </button>
        </form>
      </div>
